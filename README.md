...
3. Open an existing `.ino` file or edit the starter sketch. Saving is optional for compilation. Select **Blocks** beside **Code** above the editor to build a sketch visually. Code and blocks synchronize live in both directions. Includes, directives, comments, declarations, assignments, library calls, functions, returns, conditions, and loops translate into individual blocks alongside the dedicated Arduino blocks. Unrecognized syntax remains in a small **Unparsed C++** block so conversion never silently drops code.
